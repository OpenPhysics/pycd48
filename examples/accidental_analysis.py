...
def measure_rates(
    cd48: CD48, num_samples: int = 30, interval: float = 2.0
...
def analyze_coincidences(
    data: Dict[str, npt.NDArray[np.floating]], tau: float = 25e-9
...
def print_results(results: Dict[str, Dict[str, Union[float, int]]]) -> None:
    ...
    if results["AB"]["significance"] > 5:
        print(
            f"  ✓ True coincidences are highly significant ({results['AB']['significance']:.1f}σ)"
        )
...
def plot_results(
    data: Dict[str, npt.NDArray[np.floating]], results: Dict[str, Dict[str, Union[float, int]]]
) -> None:
    ...
    measured = [
        results["AB"]["mean_coinc"],
        results["AC"]["mean_coinc"],
        results["ABC"]["mean_coinc"],
    ]
    accidental = [
        results["AB"]["accidental"],
        results["AC"]["accidental"],
        results["ABC"]["accidental"],
    ]
    true = [results["AB"]["true_coinc"], results["AC"]["true_coinc"], results["ABC"]["true_coinc"]]
    ...
    ax3.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Mean: {results['AB']['mean_coinc']:.2f} Hz",
    )
    ax3.axvline(
        results["AB"]["accidental"],
        color="orange",
        linestyle="--",
        linewidth=2,
        label=f"Accidental: {results['AB']['accidental']:.2f} Hz",
    )
    ...
    ax4.plot(product_range, theoretical, "r--", linewidth=2, label="Expected (accidentals only)")
